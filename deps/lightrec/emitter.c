 ...
 * registers, skip the wrapper block and lightrec_rw entirely.  The cycle
 * contract is emitted inline: current_cycle synced before the call (the
 * devices are cycle-accurate), the down-counter rebuilt from a possibly
 * updated target_cycle after it. */
