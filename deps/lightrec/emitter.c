...
/* HW-tagged plain loads/stores as direct C calls. 0 leaves them on the
 * generic wrapper path; the plugin-side IRQ register fast path is
 * independent of this. */
#define HW_DIRECT_LOAD 1
#define HW_DIRECT_STORE 1
		      ...
		      jit_code_t code, jit_code_t swap_code)
{
	...
	case LIGHTREC_IO_DIRECT_HW:
		...
		break;
	case LIGHTREC_IO_HW:
		if (HW_DIRECT_STORE && !is_swc2
		    && rec_store_hw_call(state, block, offset))
			break;
			      ...
			      const struct block *block, u16 offset)
{
	...
	/* Pushed straight from where they sit: jit_pushargr moves each
	 * argument into r4.., and this backend's register cache never hands
	 * those out (temps are r2/r3, guest registers r8-r13), so no
	 * argument can be clobbered by a later push. Staging through r2/r3
	 * instead - as the tree this came from had to, its temporaries being
	 * r4-r7 - is what breaks here: they are allocator temporaries, and
	 * jit_prepare() may take one as scratch. */
	jit_prepare();
	jit_pushargr(tmp);
	jit_pushargr(rt);
	jit_pushargr(LIGHTREC_REG_STATE);
		     ...
		     bool is_unsigned)
{
	...
	case LIGHTREC_IO_HW:
		if (HW_DIRECT_LOAD
		    && rec_load_hw_call(state, block, offset, is_unsigned))
			break;
