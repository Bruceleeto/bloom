...
static void dc_vout_set_mode(int w, int h, int raw_w, int raw_h, int bpp)
{
	...
	/* When the matrix is shared with the code generator, loading it here
	 * would clobber whatever the GTE left in XMTRX, and the renderer loads
	 * it for itself around each flush instead. */
