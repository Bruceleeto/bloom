/* SH-4 instruction encoder: one function per instruction form, each emitting
 * one 16-bit word little-endian.  Register arguments are 0-15; displacement
 * arguments are already scaled (@(disp,Rn) longword forms take disp/4, word
...
typedef struct {
        ...
        /* One byte per emitted word naming the emitter site that wrote it, for
         * profiles.  NULL on the measuring pass and in the unit tests. */
        uint8_t *tagp;
        uint8_t  tag;
} sh4_codegen;
...
extern unsigned long fgl_tag_bytes[];
...
static inline void sh4_word(sh4_codegen *cg, uint16_t w)
{
        ...
        cg->ptr += 2;
        if (cg->tagp) {
                /* Tags exist only on the second pass, so each word counts once. */
                fgl_tag_bytes[cg->tag] += 2;
                *cg->tagp++ = cg->tag;
        }
}

/* Operand field placement: n is bits 11:8, m is bits 7:4. */
...
/* Used only by the GTE: the rotation matrix lives in the back bank as XMTRX
 * for `ftrv`, and `frchg` gets in and out within one command.  FPSCR is
 * single precision, round-to-nearest, set once: no `fschg`, no doubles. */
