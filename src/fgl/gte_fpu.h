...
/* Leaf commands: called by generated code directly (`jsr`), no shim, no C
 * ABI.  The routine finds the file at GBR + FGL_AT_CP2D * 4 and preserves
 * everything but r0, r1, PR, MAC and T (gte_rtp.S).  Nonzero is the index. */
...
/* Drop everything cached from the control file: required after any write to
 * CP2C that bypasses CTC2 (reset, savestate load). */
