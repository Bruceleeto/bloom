...
/* database.c: LIGHTREC_OPT_INV_DMA_ONLY set per disc ID (Spyro hack). */
                ...
                case IR_LWC2: case IR_SWC2:
                        ...
                        case 6:         /* the aligned stores only */
                                if (p->op == IR_STORE)
                                        p->io = FGL_IO_UNKNOWN;
                                break;
                        case 7:         /* aligned loads the optimiser proved
                                         * are RAM: lockstep still compares
                                         * the block (its lightrec flags say
                                         * no device), so a wrong IR_RW shows */
                                if (p->op == IR_LOAD && io == LIGHTREC_IO_RAM)
                                        p->io = FGL_IO_UNKNOWN;
                                break;
                        case 8:         /* aligned loads from other memory */
                                if (p->op == IR_LOAD &&
                                    (io == LIGHTREC_IO_DIRECT || io == LIGHTREC_IO_BIOS ||
                                     io == LIGHTREC_IO_SCRATCH || io == LIGHTREC_IO_DIRECT_HW))
                                        p->io = FGL_IO_UNKNOWN;
                                break;
                        case 9:         /* LWC2 */
                                if (p->op == IR_LWC2)
                        ...
                        case 10:        /* LWL/LWR */
                                ...
                                break;
                        case 11:        /* HW loads */
                                if (p->op == IR_LOAD && io == LIGHTREC_IO_HW)
                                        p->io = FGL_IO_UNKNOWN;
                                break;
                        default:
                                break;
...
static int lower_slot(ir_ctx *c, const struct opcode *op, uint32_t pc)
{
        ...
        default:
                 ...
                 *
                 * The impossible-branch case above no longer reaches here --
                 * a transfer in a slot ends the block early instead -- but
                 * everything else the decoder cannot lower still does.
                 *
                 * NOT IN BLOOM.  Do not drop it when syncing. */
                c->unknown = 0;
...
/* CARRY lightrec's IDLE-LOOP VERDICT ONTO THE BRANCH NODE.
 *
 * `lightrec_detect_idle` proves that a local backward branch closes a loop
 * nothing but an interrupt can break out of.  The proof is expensive and
 * lightrec has already paid for it; all fgl has to do is not throw it away.
 * The emitter turns the hint into a budget clamp on the taken edge
 * (`emit_idle_clamp`), so the loop is waited out instead of spun.
 *
 * The flag belongs on the transfer node -- the one carrying the two targets --
 * and `ir_decode_transfer` may append several nodes for one branch, so the
 * search is over everything it added. */
unsigned long fgl_idle_marked, fgl_idle_lost;

static void mark_idle_loop(ir_ctx *c, int mark)
{
        int i;
        /* FGL_NO_IDLE=1: leave the loops to spin, so a run with the clamp can
         * be diffed against one without it.  An optimisation that changes a
         * RAM hash is a bug, and this is how that is checked. */
        static int off = -1;

        if (off < 0)
                off = getenv("FGL_NO_IDLE") != NULL;
        if (off)
                return;

        for (i = mark; i < c->n; i++) {
                if (c->out[i].op == IR_COND || c->out[i].op == IR_JUMP) {
                        c->out[i].hint |= FGL_H_IDLE;
                        fgl_idle_marked++;
                        return;
                }
        }

        /* lightrec said idle loop and the lowering produced no transfer node
         * to hang it on.  Nothing breaks -- the loop just spins as before --
         * but it means the two ends disagree about what a branch lowers to,
         * so it is worth knowing rather than silently losing. */
        fgl_idle_lost++;
}
                ...
                if (op_flag_local_branch(op->flags)) {
                        ...
                        if (ir_decode_transfer(&c, word, branch_at)) {
                                ...
                                if (!op_flag_no_ds(op->flags) && i + 1 < nb) {
                                         ...
                                         * access in that slot was then
                                         * performed against the flat map: the
                                         * store went nowhere and the load
                                         * read a dead array.
                                         *
                                         * NOT IN BLOOM.  Do not drop it. */
