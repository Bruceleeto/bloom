 ...
 *
 * **The rounding is not the shift.**  The reference builds MAC with `>> 12` on
 * a signed 64-bit value, which floors.  `ftrc` truncates toward zero.  They
 * differ by one on every negative with a fraction, which is half of all
 * vertices, so the floor is corrected explicitly below.  Measured: 23% of
 * results match without the correction and 99.8% with it (`docs/tests/
 * test_gte.c`, gotcha 40).
 *
...
/*
 * One vertex: transformed, floored, stored to MAC, clamped into IR.
 *
...
/*
 * One vertex through the resident matrix, floored, as three s32.
 *
 * The floor: `ftrc` gives the truncation, `float` puts it back, and a compare
 * says whether it moved the wrong way.  `subc` against a zero register
 * subtracts T without a branch, which is why r0 is parked at zero.
 *
 * Only fr0-fr4, r0, r1, FPUL and T are touched, all caller-saved, so this
 * needs no prologue.
 */
static void gte_xform(const s16 *v, s32 *mac)
{
	__asm__ __volatile__(
		...
		:
		: [v] "r" (v), [mac] "r" (mac)
		: "r0", "r1", "fr0", "fr1", "fr2", "fr3", "fr4",
		  "fpul", "t", "memory");
}

/*
 * Three floats to three floored 32-bit results.
 *
 * The same compare-and-subtract-T floor the transform uses, factored out for
 * the commands whose products are three multiplies rather than a matrix: the
 * caller does the arithmetic in C floats and this does the narrowing, because
 * `ftrc` truncates toward zero where the reference's `>>` floors.
 *
 * The scratch is an output operand rather than a named register, so the
 * compiler cannot place one of the three inputs on top of it.
 */
 ...
 */
/*
 * Floor a float, without a library call and without a branch.
 *
 * Biasing into the positives makes truncation toward zero *be* the floor, so
 * this is an add, an `ftrc` and a subtract - `floorf()` would be a call into
 * libm, and the compare-and-decrement form the transform uses costs five.
 * Valid while the value stays inside +-65536, which the caller's own clamp
 * guarantees: screen coordinates are limited to +-1024 immediately after.
 */
static s32 gte_floor_small(float v)
{
	return (s32)(v + 65536.0f) - 65536;
}
...
gte_rtp_one_i(psxCP2Regs *r, int v, u16 *szp, s16 *sxp, s16 *syp)
{
	...
	/*
	 * The `ftrc` saturation flags, which the fused routine does not raise:
	 * they are three compares against the two extremes and they fire on
	 * nothing a game reaches, so they stay out of the hot path and read
	 * MAC back out of the file.
	 */
	AF1(D_MAC1);
	AF2(D_MAC2);
	AF3(D_MAC3);
