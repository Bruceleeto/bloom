 ...
 *
 * XMTRX belongs to this file.  It otherwise holds
 ...
 */
#if (defined(__sh__) || defined(GTE_HOST_ASM)) && defined(GTE_RTP_SELFTEST)
 ...
 * is its XMTRX; the C stand-ins here keep reading gte_resident, so both
 * halves of the self-test see the same matrix. */
...
static void gte_xform(const s16 *v, s32 *mac)
{
	...
	for (i = 0; i < 3; i++) {
		 ...
		 * (gte_sh4.c), which accumulates in double and rounds once;
		 * sum the same way so the self-test compares the assembly's
		 * plumbing and not two summation orders. */
		float acc = (float)((double)m[i] * v[0] +
				    (double)m[4 + i] * v[1] +
				    (double)m[8 + i] * v[2] +
				    (double)m[12 + i]);
#else
...
static inline void gte_dispatch(psxCP2Regs *r, u32 op)
{
	...
	/* OP has no assembly, but it still has to be dispatched: it sits in
	 * the `#else` list below and was not repeated here, so on the target
	 * every OP reaching this switch fell through to `default` and did
	 * nothing.  Found by running the assembly under the SH-4 interpreter
	 * against the integer GTE (2998 OPs in a Spyro bench scene, all
	 * wrong); the recompiler never saw it because gte_fpu_resolve()
	 * returns gte_op for 0x0c and calls it directly. */
...
#elif defined(__sh__) || defined(GTE_HOST_ASM)
